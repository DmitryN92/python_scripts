...
Тесты для .mp4 25.02.21:
    временно поменял в сценарии проверку .jpg на .mp4 (см. ниже)
    При надобности поменять обратно.
    изменение байта в индекс+20 дал результат в видео длительностью 3 секунды:
    получено искажение кадров в начале видео vid_test.mp4
...
if __name__ == '__main__':
    ...
    if '--remote' in sys.argv:
        ...
        if '_new' in fname:
            ...
            if '.mp4' in fname: # временно заменил на .mp4 !
                fname = fname.replace('.mp4', '_new%s.mp4' % shift)
            else:
    ...
    if '_new' in fname:
        ...
        if '.mp4' in fname: # replace to .jpg
            fname = fname.replace('.mp4', '_new.mp4')
        else:
